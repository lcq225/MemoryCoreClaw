...
### 🔒 安全操作（v2.1.0 新增）
- **SafeMemory 接口** - 连接管理、事务保护、边界检查
- **核心记忆保护** - 防止误删重要记忆
- **来源标记** - 追踪记忆来源（user/llm/document/system）
- **健康检查** - 自动检测并修复孤立记录

### 🏛️ 本体论与知识图谱（v2.5.0 新增）
- **内置本体论** - 类层次结构、属性继承、关系推理，支持 28+ 种关系类型
- **实体检测** - 自动从文本中识别实体类型（人物、组织、技术、项目等），提供置信度评分
- **关系推理** - 智能提取和推断实体间关系，支持传递性和对称性推理
- **增强搜索** - 结合关键词、语义和关系感知的综合记忆检索
- **知识三元组** - 主-谓-宾结构化的知识存储，实现精准知识表达
...
mem.remember(
    ...
    importance=0.85,
    category="preference",
    source="user",           # 来源：user/llm/document/system
    source_confidence=1.0    # 来源置信度
)

# 带边界检查的检索（自动修正无效参数）
results = mem.recall("", limit=-1)  # 自动修正为 limit=10

# 带保护的删除（核心记忆需要确认）
result = mem.delete(1)              # 如果是核心记忆，返回警告
result = mem.delete(1, force=True)  # 强制删除
```
...
# 同步到 Markdown（人类可读）
python scripts/sync_to_memory_md.py
...
│   │   ├── engine.py        # 记忆引擎
│   │   ├── memory.py        # 统一接口
│   │   ├── entity_detector.py      # 实体检测器 (v2.5.0)
│   │   ├── relation_inferencer.py  # 关系推理器 (v2.5.0)
│   │   └── ontology_integration.py # 本体论集成 (v2.5.0)
│   ├── cognitive/           # 认知模块
│   │   ├── forgetting.py    # 遗忘曲线
│   │   ├── contextual.py    # 情境记忆
...
│   ├── safe_memory.py       # 安全接口 (v2.1.0)
│   └── core/plugin_system.py # 插件系统 (v2.4.0)
├── scripts/                 # 维护脚本 (v2.1.0)
│   ├── check_memory.py      # 数据库检查
│   ├── optimize_database.py # 数据库优化
│   ├── sync_to_memory_md.py # 同步导出
│   └── ...                  # 更多脚本
├── docs/                    # 文档
